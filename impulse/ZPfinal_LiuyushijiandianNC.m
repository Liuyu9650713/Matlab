...
[x,fval,EXITFLAG]=fmincon(@Z_objgrade,x0,A,b,[],[],lb,ub,[],options)
...
%% 协态方程
function dy=Z_costate(t,y,tt,state1,state2,state3,state4,state5)
lambda=0.0115;eta=0.247e-4;r=0.0026;
...
dy=(-1)*[lambda*y(1)-eta*y(1)*y3+lambda*y(2)+eta*y(2)*y3-2*eta*y(2)*y5+lambda*y(3)+lambda*y(4)+lambda*y(5)-eta*y(5)*y4;
...
rho1=1;rho2=1;A=100;delta=0.1;d=8;beta=1.55;sigma=100;xi=150;
...
obj_conS=[]; 
 ...
 for i=1:N+1;                                         %第一部分：调用状态函数，求解每一点的状态
         ...
         if i<N+1
              yyb=max(yb(i,1)+norminv(1-delta)*sqrt(yy(i,2))-xi,0).^2;    %列向量，约束违反函数中的约束 
              obj_conS=[obj_conS yyb];                           %行向量
...
inv_lamdai0=[0 0 rho2 0 0];                                 %协态的在最后一段时间点1上的初始值
 ...
 for i=1:N+1;
     ...
     [inv_tti,inv_lamdai]=ode45(@(t,y)Z_costate(t,y,ti,yi(:,1),yi(:,2),yi(:,3),yi(:,4),yi(:,5)),inv_tspan, inv_lamdai0);
