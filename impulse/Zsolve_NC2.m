...
N=5;
x0=zeros(1,N+1);
 x0(1:N)=[0.1 0.2 0.1 0.3 0.2];
% x0(1:N)=[0.16 0.27 0.32 0.295 0.42];
x0(end)=0.1;
...
dy=(-1)*tau*[2*epsion^(-d)*max(y1+norminv(1-de)*sqrt(y2-y1^2)-xi,0)*(1-norminv(1-de)*y1/sqrt(y2-y1^2))+lambda*y(1)-eta*y(1)*y3+lambda*y(2)+eta*y(2)*y3-2*eta*y(2)*y5+lambda*y(3)+lambda*y(4)+lambda*y(5)-eta*y(5)*y4;
    2*epsion^(-d)*max(y1+norminv(1-de)*sqrt(y2-y1^2)-xi,0)*norminv(1-de)*1/(2*sqrt(y2-y1^2))+2*lambda*y(2)-2*eta*y(2)*y3+lambda*y(5);
